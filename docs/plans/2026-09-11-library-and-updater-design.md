...
- **分頁取代分組**：看過分組版後改成「佇列」「已完成」兩個分頁，進行中的不被舊項目淹掉、瀏覽舊項目時也不用看進度。
- **移除只留 ×**：試過往左滑掉，手勢跟捲動打架，拿掉。還沒完成的移除前在列內先問——那等於取消下載。取消要殺整個 process group（yt-dlp 是 PyInstaller 包的，會再 fork 一層）。
- **清除持久化**：「清掉已完成」之前只清記憶體、重開就回來，改成跟單筆移除一樣寫 tombstone。
- **失敗可重試**：同一列同一個 id 再走一次；歷史同 id 只算最後那筆。
